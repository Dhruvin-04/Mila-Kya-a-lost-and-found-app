      ...
      <ScrollView className="flex-1">
        ...
        <View className="px-4 py-4">
          ...
          <View className={`self-start px-3 py-1 rounded-full mb-3 ${
            item.type === 'lost' ? 'bg-orange-100' : 'bg-green-100'
          }`}>
            <Text className={`text-sm font-semibold ${
              item.type === 'lost' ? 'text-orange-700' : 'text-green-700'
            }`}>
          ...
          <View className="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <Text className="text-sm text-gray-600">student@college.edu</Text>
          </View>

          {/* Action Buttons */}
          <View>
            <Button
              onPress={() => navigation.navigate('Chat', { itemId: item.id })}
              variant="primary"
              size="lg"
              className="mb-3"
            >
